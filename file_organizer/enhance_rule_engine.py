#!/usr/bin/env python3
"""
Enhance rule_engine.py with numeric comparisons and complex conditions.
"""
import re
import sys
from pathlib import Path

source_path = 'src/rule_engine.py'
with open(source_path, 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the line numbers of key methods
...
# Insert new methods after _evaluate_condition
eval_cond_start = find_method('_evaluate_condition')
if eval_cond_start == -1:
    print("Error: _evaluate_condition not found")
    sys.exit(1)

# Find the end of _evaluate_condition (next method or end of class)
indent = None
for i in range(eval_cond_start + 1, len(lines)):
    line = lines[i]
    if line.strip() == '':
        continue
    if line.startswith(' ' * 4) and not line.startswith(' ' * 8):
        # This line is at class level (4 spaces) not inside method (8 spaces)
        # Actually inside method lines have 8 spaces (since class level + method level)
        # We need to detect when indentation returns to 4 spaces.
        # Let's compute indentation length.
        if indent is None:
            # Determine indentation of method body
            pass
    # Simpler: find line with 'def ' at same indentation as class methods
    if line.strip().startswith('def ') and line.startswith(' ' * 4):
        # Next method start
        eval_cond_end = i
        break
else:
    eval_cond_end = len(lines)

# Instead of complex insertion, we'll replace the whole file with a new version.
# Let's just write a new version with enhancements.
print("Creating enhanced version...")

# We'll rewrite the whole file.
new_content = '''"""
...
print("Enhanced rule_engine.py written.")